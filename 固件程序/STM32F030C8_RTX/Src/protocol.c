...
//应用通信协议缓存结构体变量
UartRec_t uart1RecBuff=
{
	.buff = lock_uart_buff,
	.state = SENSOR_FRAME_HEAD, 
};
...
void FrameRouteData_74Convert(uint8_t *srcData, uint8_t srcLen, uint8_t *outLen, uint8_t mode)
{
  ...
  if (mode == 0) //解码
  {
    ...
    frame_len = _74DecodeBytes((uint8_t *)&srcData[5], temp, _74ConvertLen); //把编码过后加的CRC16(2个字节)去掉
		srcData[2] = frame_len +3;
		srcData[3] = ~srcData[2];
		temp[frame_len] = 0x96;
		temp[frame_len+1] = 0x96;
		memcpy((uint8_t *)&srcData[5], temp, frame_len+2);
  ...
  else //编码
  {
    ...
    frame_len = _74CodeBytes((uint8_t *)&srcData[5], temp, _74ConvertLen); //74编码

    memcpy((uint8_t *)&srcData[5], temp, frame_len); //把编码好的数据复制回原来数据的缓存区，
		srcData[2] = frame_len+5;
		srcData[3] = ~srcData[2]; 
    crc16 = CRC16_2((uint8_t *)srcData, frame_len + 5);     //编码后长度+协议帧前面没编码的8个字节帧数据

    srcData[frame_len+5] = (crc16 >> 8);
    srcData[frame_len+5+1] = (crc16 & 0x00ff);
    srcData[frame_len+5 + 2] = 0x96; //编码后加上帧结束0x53
		srcData[frame_len+5 + 3] = 0x96; //编码后加上帧结束0x53
    *outLen = srcData[2] + 4;
  }
}
		...
		switch(mac_cmd->CmdFunc)
		{
			case 0x20://写mac
				//if(deviceInfo.mac_exist != 0x01)	//还没有MAC地址
				{
					...
					memcpy(deviceInfo.mac,mac_cmd->mac,8);
			...
			case 0x30://读mac
				...
				UartSendBytes(USART1,(uint8_t*)mac_cmd,mac_cmd->DataLen+5);
				break;
		}
	}
}
